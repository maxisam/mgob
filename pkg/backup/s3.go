...
func minioUpload(file string, plan config.Plan, storagePath string) (string, error) {
	...
	message := fmt.Sprintf("%v -> %v/%v", file, plan.S3.Bucket, fileName)
	if output != "" {
		message = fmt.Sprintf("%v %v", message, strings.Replace(output, "\n", " ", -1))
	}
...
func minioMirror(plan config.Plan, storagePath string, timeout time.Duration) (string, error) {
	...
	message := fmt.Sprintf("mirror --remove %v %v", planDir, plan.S3.Bucket)
	if output != "" {
